# src/ui/components/karakter_view.py
"""
Tek karakter görünümü component'i - büyük ve detaylı karakter bilgileri
"""
...
class SingleKarakterView(ctk.CTkFrame):
    ...
    def _create_info_section(self):
        ...
        # Temel bilgiler
        self._add_info_row(info_frame, 0, "Ölçü:", "dim_value")
        ...
        self._add_info_row(info_frame, 4, "Açıklamalar:", "remarks_value", wraplength=400)
        
        # Parsed dimension bilgileri
        self._add_info_row(info_frame, 5, "Tolerance Tipi:", "tolerance_type_value", text_color="lightblue")
        self._add_info_row(info_frame, 6, "Nominal Değer:", "nominal_value", text_color="lightgreen")
        self._add_info_row(info_frame, 7, "Tolerance Sınırları:", "limits_value", text_color="yellow")
    
    def _add_info_row(self, parent, row: int, label_text: str, value_attr: str, 
                      wraplength: int = 300, text_color: str = None):
        """Bilgi satırı ekler"""
        # Label
        label = ctk.CTkLabel(parent, text=label_text, font=ctk.CTkFont(size=16, weight="bold"))
        label.grid(row=row, column=0, sticky="w", padx=20, pady=10)
        
        # Value
        value_font_size = 16 if row < 5 else 14  # Parse edilen bilgiler daha küçük font
        sticky = "nw" if row == 4 else "w"  # Remarks için northwest
        ...
        value_label = ctk.CTkLabel(
            ...
            justify="left" if row == 4 else "center"
        )
        
        if text_color:
            value_label.configure(text_color=text_color)
        
        value_label.grid(row=row, column=1, sticky=sticky, padx=20, pady=10)
    ...
    def _create_measurement_section(self):
        ...
        title.grid(row=0, column=0, columnspan=3, pady=(15, 10))
        ...
        # Butonlar
        self._create_measurement_buttons(measurement_frame)
    ...
    def _create_measurement_buttons(self, parent):
        """Ölçüm butonlarını oluşturur"""
        # Kaydet butonu
